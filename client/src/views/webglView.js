  ...
  // FOR EXPLORATION MODE
          PubSub.subscribe('Countries:restcountriesAPI-data', (event) => {
            ...
            explorationData.forEach((country) => {
              ...
              marker.bindPopup(`<b>${country.name}<br><img src='${country.flag}' height='40' width='60'><br>Capital: ${country.capital}<br>Population: ${country.population}<br>Currency: ${country.currencies[0].name}`, {maxWidth: 150, closeButton: false});
            }
          )
          });
...
// ANIMAT
          ...
          //
          // let before = null;
          //         request = requestAnimationFrame(function animate(now) {
          //             var c = map.getPosition();
          //             var elapsed = before? now - before: 0;
          //             before = now;
          //             map.setCenter([c[0], c[1] + 0.1*(elapsed/30)]);
